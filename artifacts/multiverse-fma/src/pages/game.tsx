    ...
    return (
      <Layout>
        <div className="min-h-[70vh] flex flex-col items-center justify-center gap-4 text-center px-4">
          ...
          <p className="font-heading text-muted-foreground text-xl">{t("game.error")}</p>
          <button 
            onClick={() => refetch()}
              ...
              >
                {submitMutation.isPending ? (
                  <span className="flex items-center gap-4">
                    <Loader2 className="w-8 h-8 animate-spin" /> {t("game.submitting")}
                  </span>
                ) : (
                  <>
                    <span className="relative z-10">{t("game.submit")}</span>
